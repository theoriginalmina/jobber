const AUTH_API_URL = 'http://localhost:3000/auth/graphql';
...
const JOBS_API_URL = 'http://localhost:3001/jobs/graphql';
...
async function executeJobWithInputs(executeJobInput, cookies) {
...
(async () => {
  ...
  if (loginData?.data.login.id) {
    ...
    const data = await executeJobWithInputs(executeJobInput, cookies);
    console.log(data);
  } else {
